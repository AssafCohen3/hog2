...
MNPuzzleState<4, 4> GetMyKorfInstance(int which) {
...
void MyTest(uint64_t pN, uint64_t alg, double weight, uint64_t alternationRateForward, uint64_t alternationRateBackward) {
    MNPuzzle<4, 4> mnp;
	MNPuzzleState<4, 4> start, goal;
	goal.Reset();
	start = GetMyKorfInstance(pN);
	...
	if(alg == 1){
		...
		timer.StartTimer();
		mm.setWeight(weight);
		mm.setAlternationRateForward(alternationRateForward);
		mm.setAlternationRateBackward(alternationRateBackward);
		mm.GetPath(&mnp, start, goal, &mnp, &mnp, solutionPath);
		timer.EndTimer();
		expandedNodes = mm.GetNodesExpanded();
	}
	printf("%1.0f,%llu,%1.2f\n",
                   mnp.GetPathLength(solutionPath),
                   expandedNodes, timer.GetElapsedTime());
	exit(0);
}
